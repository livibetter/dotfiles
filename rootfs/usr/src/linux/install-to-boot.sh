...
BOOTIMG=/boot/kernel-$(uname -m)-$VERSION-gentoo$CONFIG_LOCALVERSION
...
root (hd0,0)\\
kernel /boot/kernel-$(uname -m)-$VERSION-gentoo$CONFIG_LOCALVERSION root=/dev/sda3 quiet\\
\\
title Gentoo Linux $VERSION$CONFIG_LOCALVERSION (recuse)\\
root (hd0,0)\\
kernel /boot/kernel-$(uname -m)-$VERSION-gentoo$CONFIG_LOCALVERSION root=/dev/sda3 init=/bin/bb
" /boot/grub/grub.conf
fi
